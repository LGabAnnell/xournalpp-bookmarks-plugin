...
function get_mouse_position()
  local ok, x, y = pcall(function()
    ...
    print("========== LGI loaded ===========")
    local display = lgi.Gtk.Display.get_default()
...
function dialog_new_bookmark()
  ...
  local function bookmark_name_from_entry()
    local name = ui.entryName:get_text():match("^%s*(.-)%s*$")
    if name == "" then return nil end

    local prefix, content = parse_bookmark(name)
    if prefix then
    ...
    -- A plain name is a top-level bookmark. Keep the prefix in the document
    -- text so navigation and export can recognize it later.
    return "* " .. name
  end

  function ui.entryName:on_changed()
    ui.btnNewOk:set_sensitive(bookmark_name_from_entry() ~= nil)
  end

  local function ok()
    local name = bookmark_name_from_entry()
    if name then
